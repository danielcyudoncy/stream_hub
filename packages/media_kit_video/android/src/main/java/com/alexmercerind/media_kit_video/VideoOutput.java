...
public class VideoOutput {
                ...
                surfaceTextureEntry.surfaceTexture().setOnFrameAvailableListener((texture) -> {
                    synchronized (lock) {
                        try {
                            ...
                            FlutterJNI flutterJNI = getFlutterJNIReference();
                            if (disposed || flutterJNI == null) {
                                // The engine has been detached (e.g. activity teardown) or the
                                // video output has been disposed. Drop the frame instead of
                                // spinning on a stale FlutterJNI reference.
                                return;
                            }
    ...
    public void dispose() {
        ...
        try {
            ...
            // Ignore: the SurfaceTexture may already be released or the listener never set.
        }
        try {
    ...
    private FlutterJNI getFlutterJNIReference() {
        try {
            // The [MediaKitVideoPlugin.activity] field may be null once the FlutterActivity is
            // destroyed. Engine teardown is a normal event during playback shutdown; a missing
            // reference must not surface as exceptions.
            if (MediaKitVideoPlugin.activity == null) {
                return null;
            }
            ...
            if (engine == null) {
                // The engine is no longer attached to the view during teardown.
                return null;
            }
            ...
            Log.i("media_kit", "VideoOutput.getFlutterJNIReference: " + e);
            return null;
        }
    }
}
