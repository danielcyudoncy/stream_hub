// test/shared/tv_scaffold_focus_repro_test.dart
...
Widget _wrap(Widget body) {
  return GetMaterialApp(home: TvScaffold(body: body));
}
...
void main() {
  ...
  group('TvScaffold CTA focus', () {
    testWidgets('autofocused CTA in body receives initial focus', (
      tester,
    ) async {
      ...
      expect(
        ...
        ctaNode,
        reason:
            'autofocused CTA (not the invisible body container) should be focused',
      );
    ...
    testWidgets(
      'D-pad Right from sidebar exits to the CTA, not the bare body node',
      (tester) async {
        tester.view.physicalSize = const Size(1920, 1080);
        tester.view.devicePixelRatio = 1.0;
        addTearDown(tester.view.resetPhysicalSize);
    ...
    testWidgets(
      'sidebar exit with no prior body focus lands on the first body focusable',
      (tester) async {
        tester.view.physicalSize = const Size(1920, 1080);
        tester.view.devicePixelRatio = 1.0;
        addTearDown(tester.view.resetPhysicalSize);
        ...
        expect(
          ...
          isTrue,
          reason:
              'Sidebar Right must land on the FIRST actionable body focusable',
        );
    ...
    testWidgets(
      'body container node can never be the primary focus (no invisible target)',
      (tester) async {
        tester.view.physicalSize = const Size(1920, 1080);
        tester.view.devicePixelRatio = 1.0;
        addTearDown(tester.view.resetPhysicalSize);
        ...
        expect(
          cardNode.hasFocus,
          isFalse,
          reason: 'Left at boundary opens the sidebar',
        );
    ...
    testWidgets(
      'back from the sidebar restores the first actionable body target',
      (tester) async {
        tester.view.physicalSize = const Size(1920, 1080);
        tester.view.devicePixelRatio = 1.0;
        addTearDown(tester.view.resetPhysicalSize);

        final firstBodyAction = FocusNode(debugLabel: 'First Action');
        final secondBodyAction = FocusNode(debugLabel: 'Second Action');
        ...
        await tester.pumpWidget(
          _wrap(
            Column(
              children: [
                TvFocusable(
                  focusNode: firstBodyAction,
                  onTap: () {},
                  child: const Text('First Action'),
                ),
                TvFocusable(
                  focusNode: secondBodyAction,
                  onTap: () {},
        ...
        await tester.pumpAndSettle();
        expect(firstBodyAction.hasFocus, isTrue);

        await tester.sendKeyEvent(LogicalKeyboardKey.arrowLeft);
        await tester.pump();
        expect(firstBodyAction.hasFocus, isFalse);

        await tester.sendKeyEvent(LogicalKeyboardKey.gameButtonB);
        await tester.pump();

        expect(
          firstBodyAction.hasFocus,
          isTrue,
          reason:
              'Back from the sidebar should restore the first actionable body target',
        );

        firstBodyAction.dispose();
        secondBodyAction.dispose();
      },
    );
  });
}
